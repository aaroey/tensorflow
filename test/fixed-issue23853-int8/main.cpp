...
using ImportGraphDefOptionsPtr =
    std::unique_ptr<TF_ImportGraphDefOptions, ImportGraphDefOptionsDeleter>;
...
using SessionOptionsPtr =
    std::unique_ptr<TF_SessionOptions, SessionOptionsDeleter>;
...
SessionPtr MakeSession(TF_Graph* graph) {
  ...
  TF_SetConfig(
      options.get(), session_config, sizeof(session_config), status.get());
...
int main() {
  ...
  const std::array<TF_Output, 10> outputs{GetOutput(graph.get(), "output_0"),
  ...
  const auto value = std::unique_ptr<float[]>(
      new float[dims[0] * dims[1] * dims[2] * dims[3]]);

  std::array<TensorPtr, 1> input_tensors{TensorPtr(TF_NewTensor(
      TF_FLOAT,
      /* dims */ &dims[0],
      dims.size(),
      /* data */ const_cast<float*>(value.get()),
      sizeof(float) * dims[0] * dims[1] * dims[2] * dims[3],
    ...
    threads.emplace_back([&]() {
      ...
      for (int j = 0; j < 100; ++j) {
        ...
        TF_SessionRun(session.get(),
                      /* run_options */ nullptr,
                      &inputs[0],
                      reinterpret_cast<TF_Tensor**>(&input_tensors[0]),
                      inputs.size(),
                      &outputs[0],
                      reinterpret_cast<TF_Tensor**>(&output_tensors[0]),
                      outputs.size(),
        ...
        if (TF_GetCode(status.get()) != TF_OK) {
          std::cerr << "TF_SessionRun: " << TF_Message(status.get())
                    << std::endl;
          std::abort();
        }
      }
